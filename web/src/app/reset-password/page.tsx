...
export default function ResetPasswordPage() {
  ...
  const [btnText, setBtnText] = useState('Reconfigure Key');
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Key reconfigure failed');
      }

      setSuccess("Cryptographic key successfully updated.");
      setBtnText("Redirection Initialized");
      setTimeout(() => router.push('/login'), 2000);
    } catch (err: any) {
      setError(err.message);
      setIsScanning(false);
      setBtnText("Reconfigure Key");
    }
  };
      ...
      <div className="login-card z-10 w-full max-w-[420px] bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-12 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] transition-all duration-300">
        ...
        <div className="text-center mb-10">
          <div className="inline-block p-3 rounded-full bg-white/5 border border-white/10 mb-4 shadow-[0_0_20px_rgba(13,95,183,0.3)]">
            <i className="fa-solid fa-shield-halved text-2xl text-primary drop-shadow-[0_0_10px_#0d5fb7]"></i>
          </div>
          <h1 className="font-heading font-bold text-2xl tracking-tight">Key Reconfiguration</h1>
          <p className="text-gray-400 text-xs font-mono mt-2 uppercase tracking-widest">Establish new parameters</p>
        </div>
        ...
        <form onSubmit={handleReset}>
          ...
          <div className="admin-input-group relative mb-6">
            ...
            />
            <i className="fa-solid fa-lock absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 peer-focus:text-primary transition-colors"></i>
          </div>
